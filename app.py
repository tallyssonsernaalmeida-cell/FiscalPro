...
def add_log(acao, usuario='sistema', detalhe=''):
    ...
    if os.path.exists(LOGS_FILE):
        try:
            ...
            logs = []
    # ✅ FIX 4: chave 'ip' restaurada (era 'detalhe' — quebrava template admin)
...
def categorizar(desc):
    d = str(desc).lower()
    # Estorno/Reembolso SEMPRE primeiro — evita categoria errada
    ...
    if 'compra no débito' in d or 'compra no debito' in d or 'nupay' in d:
        ...
        if any(x in d for x in ['panificadora','padaria','cafe','café','pub',
                                  'lanchonete','restaurante','pizz']):          return 'Alimentação'
    ...
    if any(x in d for x in ['supermercado','mercado','hortifruti','açougue','acougue',
                              'padaria','restaurante','ifood','lanchonete','alimentação',
                              'alimentacao','comida']): return 'Alimentação'
...
def process_pdf(fp):
    """
    Extrai transações de PDF de extratos bancários.
    Suporte: BB, Santander (padrão DD/MM) e Itaú/Bradesco (padrão DD/MM/AAAA).
    PDFs escaneados (imagem) não são suportados.
    """
    try:
        # ✅ FIX 5: pypdf em vez de PyPDF2 (descontinuado)
        try:
            from pypdf import PdfReader
        except ImportError:
            from PyPDF2 import PdfReader  # fallback se pypdf não instalado
        ...
        # Detecta mês/ano do extrato nas primeiras 20 linhas
        ...
        # ✅ FIX 1: i += 1 incondicional no final do loop — sem risco de loop infinito
        i = 0
        while i < len(linhas):
            linha = linhas[i].strip()

            # Pula linhas vazias e cabeçalhos conhecidos
            ...
            # Padrão 1: DD/MM DESCRIÇÃO VALOR  (BB, Santander)
            match = re.match(r'^(\d{2}/\d{2})\s+(.+)', linha)
            if match:
                dia_mes  = match.group(1)
                restante = match.group(2).strip()
                data     = f"{ano}-{mes}-{dia_mes.split('/')[0]}"

                # Linha seguinte pode ser continuação da descrição
                if (i + 1 < len(linhas)
                        and not re.match(r'^\d{2}/\d{2}', linhas[i+1])
                        and linhas[i+1].strip()):
                    restante += ' ' + linhas[i+1].strip()
                    i += 1

                match_val = re.search(r'([\d.,]+)\s*$', restante)
                if match_val:
            ...
            # Padrão 2: DESCRIÇÃO DD/MM/AAAA VALOR  (Itaú, Bradesco)
            if not processou:
                match_data = re.search(r'\d{2}/\d{2}/\d{4}', linha)
                match_val  = re.search(r'([\d.,]+)\s*$', linha)
                if match_data and match_val:
                    try:
                        val_str = match_val.group(1).replace('.', '').replace(',', '.')
                        valor   = float(val_str)
                        if linha.rstrip().endswith('-'):
                            valor = -abs(valor)
                        p    = match_data.group(0).split('/')
                        data = f'{p[2]}-{p[1]}-{p[0]}'
                        desc = (linha[:match_data.start()].strip() + ' ' +
                                linha[match_data.end():match_val.start()].strip()).strip()
                        desc = desc[:200] if desc else 'Sem descrição'
                        txs.append({
                            'data':      data,
                            'descricao': desc,
                            'valor':     valor,
            ...
            i += 1  # ✅ sempre avança — fim do loop, sem exceção
...
def process_ofx_universal(fp):
    """
    Processa OFX de qualquer banco brasileiro.
    Três fallbacks: ofxparse → XML direto → CSV.
    """
    txs = []
    ...
    try:
        ...
        # ✅ FIX 2: arquivo .tmp sempre removido, mesmo em caso de erro
    ...
    try:
        ...
        print(f'OFX método 3 falhou: {e}')

    return txs


def process_file(fp):
    """Ponto de entrada único para qualquer formato: PDF, OFX, XLSX, XLS, CSV."""
...
def _process_dataframe(df):
    """Converte DataFrame pandas em lista de transações normalizadas."""
...
def build_excel_conciliacao(data, recomendacoes=None):
    ...
    for i, (m, v) in enumerate(metricas):
        ...
        c2.font = Font(name='Calibri', bold=True, size=12, color=AZUL); c2.fill = _fill('F4F6F8'); c2.border = brd
    ...
    cat_end = r - 1
    if cats_d:
        bar1 = BarChart(); bar1.type = 'col'; bar1.title = 'Top Despesas'
        bar1.style = 10; bar1.width = 18; bar1.height = 12
...
def calcular_conciliacao(rec_list, desp_list):
    ...
    if total == 0:
        return {'confirmados': 0, 'com_par': 0, 'sem_par': 0,
                'percentual': 0, 'pares': [], 'total': 0}

    rec_pareados  = set()
    desp_pareados = set()
    pares = []

    # Passo 1: estornos/reembolsos com origem por valor+data
    ...
    # Passo 2: pares por valor+data+palavras em comum
    for ir, r in enumerate(rec_list):
        if ir in rec_pareados: continue
        for id_, d in enumerate(desp_list):
            if id_ in desp_pareados: continue
            if abs(r['valor'] - abs(d['valor'])) < 0.01 and r['data'] == d['data']:
                stopwords = {'pelo','para','pix','transferencia','transferência',
                             'recebida','enviada','recebido','enviado',
                             'debito','crédito','credito','conta','compra'}
                palavras_r = set(re.findall(r'[a-zA-ZÀ-ú]{4,}', r['descricao'].lower())) - stopwords
                palavras_d = set(re.findall(r'[a-zA-ZÀ-ú]{4,}', d['descricao'].lower())) - stopwords
                if palavras_r & palavras_d:
                    rec_pareados.add(ir); desp_pareados.add(id_)
                    pares.append({
...
def admin_dashboard():
    ...
    lista = [{
        ...
        'files_count': len(d.get('files', [])), 'whatsapp': d.get('whatsapp', ''),
        'created_at': d.get('created_at', '')[:10], 'ativo': d.get('ativo', True)
    ...
    return render_template('admin_dashboard.html', **ctx, active='admin',
                           ...
                           support_messages=load_support(), logs=logs_unicos[:30],
...
def conciliacao():
    ...
    return render_template('conciliacao.html', **ctx, active='conciliacao',
                           ...
                           # ✅ FIX 3: variáveis que faltavam no template
                           com_par=conc['com_par'],
                           sem_par=conc['sem_par'],
                           pares=conc['pares'],
...
def download_conciliacao():
    ...
    if redir: return redir
    dash = get_dash(session['user'])
    recs = ['✅ Conciliação 100% concluída — todos os lançamentos confirmados pelo extrato.',
