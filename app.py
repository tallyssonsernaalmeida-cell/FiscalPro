...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'fiscalpro_dev_key_2025')
...
SESSIONS_FILE      = 'sessions.json'   # ← presença online/offline

for d in [UPLOAD_FOLDER, USER_DATA, CONCILIACAO_FOLDER]:
    os.makedirs(d, exist_ok=True)

ALLOWED = {'ofx', 'xlsx', 'xls', 'xlsm', 'csv', 'pdf'}
ONLINE_TIMEOUT = 5   # minutos sem atividade = offline
...
def marcar_online(email):
    """Registra o usuário como online com timestamp atual."""
    s = load_sessions()
    s[email] = {
        'online':     True,
...
def marcar_offline(email):
    """Marca o usuário como offline."""
    s = load_sessions()
    if email in s:
        s[email]['online']    = False
        s[email]['last_seen'] = datetime.now().isoformat()
    save_sessions(s)

def get_status_usuario(email):
    """
    Retorna dict com status do usuário:
    online=True se teve atividade nos últimos ONLINE_TIMEOUT minutos.
    """
    s = load_sessions()
    if email not in s:
        return {'online': False, 'last_seen': None, 'ip': '-'}
    dados = s[email]
    if not dados.get('online', False):
        return {'online': False,
                'last_seen': dados.get('last_seen'),
                'ip': dados.get('ip', '-')}
    # Verifica timeout por inatividade
    try:
        last = datetime.fromisoformat(dados['last_seen'])
        if datetime.now() - last > timedelta(minutes=ONLINE_TIMEOUT):
            # expirou — marca offline automaticamente
            marcar_offline(email)
            return {'online': False,
                    'last_seen': dados.get('last_seen'),
                    'ip': dados.get('ip', '-')}
    except:
...
def atualizar_presenca():
    """Atualiza last_seen a cada requisição autenticada."""
    if 'user' in session:
        marcar_online(session['user'])
...
def add_log(acao, usuario='sistema', detalhe=''):
    ...
    # Deduplicação: mesmo usuário + mesma ação no mesmo dia
    vistos = set()
    unicos = []
    for log in logs[:500]:
        chave = (log['usuario'], log['acao'], log['data'][:10])
        if chave not in vistos:
            vistos.add(chave)
            unicos.append(log)
    with open(LOGS_FILE, 'w', encoding='utf-8') as f:
        json.dump(unicos[:500], f, ensure_ascii=False, indent=2)
...
def categorizar(desc):
    ...
    if 'compra no débito' in d or 'compra no debito' in d or 'nupay' in d:
        if any(x in d for x in ['pombo','pombo24']):                           return 'Delivery'
        if any(x in d for x in ['uber','99','cabify']):                        return 'Transporte'
        if any(x in d for x in ['panificadora','padaria','cafe','café',
                                  'pub','lanchonete','restaurante','pizz']):    return 'Alimentação'
        ...
        if any(x in d for x in ['pmb joi','prefeitura','municipio']):          return 'Impostos & Taxas'
    ...
    if any(x in d for x in ['supermercado','mercado','hortifruti','açougue','acougue',
    ...
    if any(x in d for x in ['uber','cabify','ônibus','onibus','metrô','metro','gasolina',
                              'combustível','combustivel','posto','estacionamento',
    ...
    if any(x in d for x in ['energia','água','agua','internet','telefone','celular',
                              ...
                              'streaming','mensalidade','celesc']): return 'Contas & Serviços'
    if any(x in d for x in ['farmácia','farmacia','drogaria','médico','medico','hospital',
                              'plano de saúde','unimed','exame','consulta',
                              'remédio','remedio']): return 'Saúde'
    if any(x in d for x in ['netflix','spotify','amazon','disney','hbo','cinema',
                              'teatro','ingresso','show','parque','viagem']): return 'Lazer'
    if any(x in d for x in ['curso','faculdade','escola','educação','educacao',
                              'livro','livraria']): return 'Educação'
    if any(x in d for x in ['roupa','calçado','calcado','moda','loja','zara',
                              'renner','riachuelo','shopping']): return 'Vestuário'
    if any(x in d for x in ['imposto','taxa','multa','darf','irpf','irpj',
                              'sefaz','municip','pmb joi','prefeitura']): return 'Impostos & Taxas'
    if any(x in d for x in ['investimento','ações','acao','fii','fundo',
                              'cdb','tesouro','bolsa']): return 'Investimentos'
    if any(x in d for x in ['seguro','previdência','previdencia','vida','auto']): return 'Seguros'
    if any(x in d for x in ['google','microsoft','office','adobe',
                              'dropbox','icloud']): return 'Assinaturas Digitais'
    return 'Outros'
    ...
    try:
        ...
        for linha in linhas[:20]:
            m = re.search(
                ...
                linha.lower())
            if m:
                mes_ano = f"{m.group(2)}-{meses_map.get(m.group(1),'01')}"
                break
        ano, mes = mes_ano.split('-')
        i = 0
        while i < len(linhas):
            linha = linhas[i].strip()
            if not linha or any(x in linha for x in [
                'Pagina:','Extrato_PF','Data Descrição','PREZADO',
                'SAC','Ouvidoria','Central de','Fale Conosco',
                'Loja:','DEBITO VISA','segurança']):
                i += 1; continue
            match = re.match(r'^(\d{2}/\d{2})\s+(.+)', linha)
            if match:
                restante = match.group(2).strip()
                data     = f"{ano}-{mes}-{match.group(1).split('/')[0]}"
                if (i+1 < len(linhas) and not re.match(r'^\d{2}/\d{2}', linhas[i+1])
                        and linhas[i+1].strip()):
                    restante += ' ' + linhas[i+1].strip(); i += 1
                mv = re.search(r'([\d.,]+)\s*$', restante)
                if mv:
                    try:
                        val_str = mv.group(1).replace('.','').replace(',','.')
                        valor   = float(val_str)
                        if restante.rstrip().endswith('-'): valor = -abs(valor)
                        desc    = restante[:mv.start()].strip()
                    except:
                        valor = 0.0; desc = restante
                else:
                    valor = 0.0; desc = restante
                txs.append({'data':data,'descricao':desc[:200],'valor':valor,
                            'tipo':'Crédito' if valor>0 else 'Débito',
                            'categoria':categorizar(desc),'pago':False})
            else:
                md = re.search(r'\d{2}/\d{2}/\d{4}', linha)
                mv = re.search(r'([\d.,]+)\s*$', linha)
                if md and mv:
                    try:
                        val_str = mv.group(1).replace('.','').replace(',','.')
                        valor   = float(val_str)
                        if linha.rstrip().endswith('-'): valor = -abs(valor)
                        p    = md.group(0).split('/')
                        data = f'{p[2]}-{p[1]}-{p[0]}'
                        desc = (linha[:md.start()].strip()+' '+
                                linha[md.end():mv.start()].strip()).strip()[:200]
                        txs.append({'data':data,'descricao':desc or 'Sem descrição',
                                    'valor':valor,'tipo':'Crédito' if valor>0 else 'Débito',
                                    'categoria':categorizar(desc),'pago':False})
                    except:
                        pass
            i += 1
        ...
        print(f'Erro PDF: {e}'); return []

def process_ofx_universal(fp):
    txs = []; temp_path = fp + '.tmp'
    try:
        with open(fp, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        content = (content
                   .replace('encoding="ISO-8859-1"','encoding="UTF-8"')
                   .replace('encoding="US-ASCII"','encoding="UTF-8"'))
        ...
        for acc in ofx.accounts:
            for t in acc.statement.transactions:
                v = float(t.amount)
                d = (t.memo or '')[:200]
                txs.append({'data':str(t.date)[:10],'descricao':d,'valor':v,
                            'tipo':'Crédito' if v>0 else 'Débito',
                            'categoria':categorizar(d),'pago':False})
    except Exception as e:
        print(f'OFX método 1: {e}')
    finally:
        ...
        for trn in root.iter('STMTTRN'):
            try:
                ...
                if dt is not None and dt.text:
                    raw=dt.text.strip()[:8]
                    if len(raw)==8: data=f'{raw[:4]}-{raw[4:6]}-{raw[6:8]}'
                valor=float(val.text.replace(',','.')) if val is not None and val.text else 0.0
                ...
                txs.append({'data':data,'descricao':descricao,'valor':valor,
                            'tipo':'Crédito' if valor>0 else 'Débito',
                            'categoria':categorizar(descricao),'pago':False})
            except: continue
        if txs: return txs
    except Exception as e:
        print(f'OFX método 2: {e}')
    try:
        df = pd.read_csv(fp, sep=None, engine='python', encoding='utf-8')
        result = _process_dataframe(df)
        if result: return result
    except Exception as e:
        print(f'OFX método 3: {e}')
    return txs

def process_file(fp):
    ext = fp.rsplit('.',1)[-1].lower()
    if ext == 'pdf': return process_pdf(fp)
    if ext == 'ofx': return process_ofx_universal(fp)
    try:
        df = None
        if ext == 'csv':
            for enc in ['utf-8','latin-1','iso-8859-1']:
                try:
                    df = pd.read_csv(fp, sep=None, engine='python', encoding=enc); break
                except: continue
        elif ext in ['xlsx','xlsm']:
            df = pd.read_excel(fp, engine='openpyxl')
        else:
            df = pd.read_excel(fp, engine='xlrd')
        if df is not None: return _process_dataframe(df)
    except Exception as e:
...
def _process_dataframe(df):
    ...
    col_d = next((c for c in df.columns
                  if any(k in c for k in ['data','date','dt','dia'])), None)
    col_v = next((c for c in df.columns
                  if any(k in c for k in ['valor','value','amount','montante','total'])), None)
    col_t = None
    for c in df.columns:
        if c.lower() in ['descrição','descricao','historico','histórico',
                          'memo','complemento','detalhe','lançamento','lancamento']:
            col_t = c; break
    if not col_t:
        for c in df.columns:
            if ('descri' in c.lower() or 'histor' in c.lower()
                    or 'lanc' in c.lower() or 'memo' in c.lower()) and 'identif' not in c.lower():
                col_t = c; break
    out = []
    for _, row in df.iterrows():
        ...
        # ✅ FIX: ignora linhas com valor zero
        if v == 0.0: continue
        data = 'N/A'
        if col_d:
            raw = str(row[col_d]).strip()[:10]
            if re.match(r'\d{2}/\d{2}/\d{4}', raw):
                p = raw.split('/'); data = f'{p[2]}-{p[1]}-{p[0]}'
            elif re.match(r'\d{4}-\d{2}-\d{2}', raw):
                data = raw
            else:
                data = raw
        desc = (str(row[col_t])[:200]
                if col_t and str(row[col_t]) not in ('nan','None','')
                else 'Sem descrição')
...
def get_dash(email):
    ...
    for t in txs:
        cat = t['categoria']
        if t['valor'] < 0: cat_d[cat] = round(cat_d.get(cat,0)+abs(t['valor']),2)
        else:              cat_r[cat] = round(cat_r.get(cat,0)+t['valor'],2)
    return {
        'transacoes':       txs,
        'transactions':     txs,
...
def build_excel_conciliacao(data, recomendacoes=None):
    ...
    ws['A1'].fill=_fill(G); ws['A1'].alignment=Alignment(horizontal='center',vertical='center')
    ...
    kpis=[('A','RECEITAS',f'R$ {tot_r:,.2f}',G,GL),
    ...
    r=8
    ws.merge_cells(f'A{r}:J{r}'); ws[f'A{r}'].value='📈 MÉTRICAS'
    ws[f'A{r}'].font=Font(name='Calibri',bold=True,size=13,color=G); r+=1
    metricas=[('Total Transações',str(data.get('qtd_transacoes',0))),
              ('Margem de Lucro',f'{pct}%'),('Status',data.get('status','N/A')),
    ...
    for i,(m,v) in enumerate(metricas):
        ...
        c1.font=Font(name='Calibri',bold=True,size=11); c1.fill=_fill('F4F6F8'); c1.border=brd
        ws.merge_cells(f'A{r+i}:C{r+i}')
        c2=ws.cell(row=r+i,column=4,value=v)
        c2.font=Font(name='Calibri',bold=True,size=12,color=AZUL)
        c2.fill=_fill('F4F6F8'); c2.border=brd; ws.merge_cells(f'D{r+i}:F{r+i}')
    r+=len(metricas)+1
    ws.merge_cells(f'A{r}:F{r}'); ws[f'A{r}'].value='🔴 TOP DESPESAS'
    ws[f'A{r}'].font=Font(name='Calibri',bold=True,size=13,color=R); r+=1
    for i,h in enumerate(['Categoria','Total (R$)','%','Qtd.','Média'],1):
        c=ws.cell(row=r,column=i,value=h); c.font=_fnt('FFFFFF',True,11); c.fill=_fill(R)
    ...
    for i,(cat,val) in enumerate(cats_d[:10]):
        ...
        media=round(val/qtd,2) if qtd>0 else 0; bg=RL if i%2==0 else 'FFFFFF'
        for ci,v in enumerate([cat,round(val,2),f'{pct_c}%',qtd,f'R$ {media:,.2f}'],1):
            c=ws.cell(row=r,column=ci,value=v); c.fill=_fill(bg); c.border=brd
            c.font=_fnt(R,True) if ci==2 else _fnt()
            c.alignment=Alignment(horizontal='left' if ci==1 else 'center',vertical='center')
        r+=1
    cat_end=r-1
    if cats_d:
        bar1=BarChart(); bar1.type='col'; bar1.title='Top Despesas'; bar1.style=10
    ...
    r=cat_end+3
    ws.merge_cells(f'A{r}:F{r}'); ws[f'A{r}'].value='🟢 TOP RECEITAS'
    ws[f'A{r}'].font=Font(name='Calibri',bold=True,size=13,color=G); r+=1
    for i,h in enumerate(['Categoria','Total (R$)','%','Qtd.','Média'],1):
        c=ws.cell(row=r,column=i,value=h); c.font=_fnt('FFFFFF',True,11); c.fill=_fill(G)
        c.alignment=Alignment(horizontal='center',vertical='center'); c.border=brd
    rec_start=r+1; r+=1
    for i,(cat,val) in enumerate(cats_r[:10]):
        ...
        media=round(val/qtd,2) if qtd>0 else 0; bg=GL if i%2==0 else 'FFFFFF'
        for ci,v in enumerate([cat,round(val,2),f'{pct_c}%',qtd,f'R$ {media:,.2f}'],1):
            c=ws.cell(row=r,column=ci,value=v); c.fill=_fill(bg); c.border=brd
    ...
    rec_end=r-1
    if cats_r:
        pie=PieChart(); pie.title='Distribuição de Receitas'; pie.style=10
    ...
    r=rec_end+20
    if recomendacoes:
        ws.merge_cells(f'A{r}:J{r}'); ws[f'A{r}'].value='💡 RECOMENDAÇÕES'
        ws[f'A{r}'].font=Font(name='Calibri',bold=True,size=14,color=AZUL); r+=1
        for rec in recomendacoes:
            ws.merge_cells(f'A{r}:J{r}'); ws[f'A{r}'].value=rec
    ...
    ws2.merge_cells('A1:F1'); ws2['A1'].value=f'💰 RECEITAS — Total: R$ {tot_r:,.2f}'
    ws2['A1'].font=Font(name='Calibri',bold=True,size=14,color='FFFFFF')
    ws2['A1'].fill=_fill(G); ws2['A1'].alignment=Alignment(horizontal='center',vertical='center')
    for i,h in enumerate(['Data','Descrição','Categoria','Valor','%','Status'],1):
        c=ws2.cell(row=2,column=i,value=h); c.font=_fnt('FFFFFF',True,11); c.fill=_fill(G)
        c.alignment=Alignment(horizontal='center',vertical='center'); c.border=brd
    for j,t in enumerate(sorted(rec_list,key=lambda x:x['data'],reverse=True)):
        rr=j+3; pct_t=round(t['valor']/tot_r*100,1) if tot_r>0 else 0
        bg=GL if j%2==0 else 'FFFFFF'
        for ci,v in enumerate([t['data'],t['descricao'][:60],t['categoria'],
                                 round(t['valor'],2),f'{pct_t}%','✅' if t.get('pago') else '⚠️'],1):
            c=ws2.cell(row=rr,column=ci,value=v); c.fill=_fill(bg); c.border=brd
    ...
    ws3.merge_cells('A1:F1'); ws3['A1'].value=f'📤 DESPESAS — Total: R$ {tot_d:,.2f}'
    ws3['A1'].font=Font(name='Calibri',bold=True,size=14,color='FFFFFF')
    ws3['A1'].fill=_fill(R); ws3['A1'].alignment=Alignment(horizontal='center',vertical='center')
    for i,h in enumerate(['Data','Descrição','Categoria','Valor','%','Status'],1):
        c=ws3.cell(row=2,column=i,value=h); c.font=_fnt('FFFFFF',True,11); c.fill=_fill(R)
        c.alignment=Alignment(horizontal='center',vertical='center'); c.border=brd
    for j,t in enumerate(sorted(desp_list,key=lambda x:x['data'],reverse=True)):
        rr=j+3; pct_t=round(abs(t['valor'])/tot_d*100,1) if tot_d>0 else 0
        bg=RL if j%2==0 else 'FFFFFF'
        for ci,v in enumerate([t['data'],t['descricao'][:60],t['categoria'],
                                 round(abs(t['valor']),2),f'{pct_t}%','✅' if t.get('pago') else '⚠️'],1):
            c=ws3.cell(row=rr,column=ci,value=v); c.fill=_fill(bg); c.border=brd
            c.alignment=Alignment(horizontal='left' if ci==2 else 'center',vertical='center')
    ...
    for ir,r in enumerate(rec_list):
        ...
        for id_,d in enumerate(desp_list):
            ...
            if abs(r['valor']-abs(d['valor']))<0.01 and r['data']==d['data']:
                ...
                pares.append({'tipo':'🔄 Estorno/Reembolso','data':r['data'],
                              'valor':r['valor'],'desc_rec':r['descricao'][:50],
                              'desc_desp':d['descricao'][:50]}); break
    stopwords={'pelo','para','pix','transferencia','transferência','recebida','enviada',
               'recebido','enviado','debito','crédito','credito','conta','compra'}
    for ir,r in enumerate(rec_list):
        if ir in rec_pareados: continue
        for id_,d in enumerate(desp_list):
            if id_ in desp_pareados: continue
            if abs(r['valor']-abs(d['valor']))<0.01 and r['data']==d['data']:
                pr=set(re.findall(r'[a-zA-ZÀ-ú]{4,}',r['descricao'].lower()))-stopwords
                pd_=set(re.findall(r'[a-zA-ZÀ-ú]{4,}',d['descricao'].lower()))-stopwords
                if pr & pd_:
                    rec_pareados.add(ir); desp_pareados.add(id_)
                    pares.append({'tipo':'🔗 Par Identificado','data':r['data'],
                                  'valor':r['valor'],'desc_rec':r['descricao'][:50],
                                  'desc_desp':d['descricao'][:50]}); break
    com_par=len(rec_pareados)+len(desp_pareados)
    return {'confirmados':total,'com_par':com_par,'sem_par':total-com_par,
            'percentual':100.0,'pares':pares,'total':total}
...
def index():
    ...
    role = load_users().get(session['user'],{}).get('role')
    ...
    if request.method=='POST':
        ...
        if email in users and check_password_hash(users[email]['password'],senha):
            if not users[email].get('ativo',True):
                ...
                session['user']=email
                marcar_online(email)   # ← marca online ao logar
            ...
            error='Email ou senha inválidos.'; prefill=email
        ...
        if not all([nome,email,pwd]):   error='Preencha todos os campos.'
            ...
            if email in users: error='E-mail já cadastrado.'
                ...
                users[email]={'password':generate_password_hash(pwd),'nome':nome,
                              'role':'user','files':[],'whatsapp':request.form.get('whatsapp','').strip(),
                              'created_at':datetime.now().isoformat(),'ativo':True}
                save_users(users)
                add_log('Novo cadastro',email,f'IP: {request.remote_addr}')
                session['user']=email
                marcar_online(email)   # ← marca online ao cadastrar
...
def logout():
    email=session.get('user','')
    if email:
        marcar_offline(email)          # ← marca offline ao sair
        add_log('Logout',email,f'IP: {request.remote_addr}')
    session.clear()
...
def admin_dashboard():
    ...
    for em,d in users.items():
        status=get_status_usuario(em)   # ← busca status online/offline
        # formata last_seen legível
        last_seen_fmt = '-'
        if status['last_seen']:
            try:
                ls = datetime.fromisoformat(status['last_seen'])
                diff = datetime.now() - ls
                if diff.total_seconds() < 60:
                    last_seen_fmt = 'Agora mesmo'
                elif diff.total_seconds() < 3600:
                    last_seen_fmt = f'{int(diff.total_seconds()//60)}min atrás'
                elif diff.total_seconds() < 86400:
                    last_seen_fmt = f'{int(diff.total_seconds()//3600)}h atrás'
                else:
                    last_seen_fmt = ls.strftime('%d/%m/%Y %H:%M')
            except:
                last_seen_fmt = status['last_seen'][:16] if status['last_seen'] else '-'
        lista.append({
            'email':       em,
            'nome':        d['nome'],
            'role':        d['role'],
            ...
            'last_seen':   last_seen_fmt,          # ← "5min atrás"
            'ip':          status['ip']            # ← IP do último acesso
        })
    # Contadores para os cards do admin
    total_online = sum(1 for u in lista if u['online'])
...
def admin_criar_usuario():
    ...
    if email in users:             flash('E-mail já cadastrado!','danger')
        ...
        users[email]={'password':generate_password_hash(pwd),'nome':nome,'role':role,
                      'files':[],'whatsapp':'','created_at':datetime.now().isoformat(),'ativo':True}
...
def toggle_ativo():
    ...
    if email in users and email!=session['user']:
        ...
        if not users[email]['ativo']:
            marcar_offline(email)  # ← força offline ao desativar
...
def remover_usuario():
    ...
    if email==session['user']:
        ...
        del users[email]; save_users(users)
        marcar_offline(email)
...
def configuracoes():
    ...
    if request.method=='POST':
        ...
        if acao=='alterar_email':
            ...
            if not novo:                               error='E-mail vazio.'
                ...
                users[novo]=users.pop(session['user']); save_users(users)
                session['user']=novo; success='E-mail alterado!'
    ...
    # ✅ FIX: nome correto do template (sem acento, minúsculo)
...
def dashboard():
    ...
    # ✅ FIX: paginar cópia, não o original
    pagina=request.args.get('pagina',1,type=int); por_pagina=50
    todas_txs=list(dash.get('transactions',[]))
    total_paginas=max(1,(len(todas_txs)+por_pagina-1)//por_pagina)
    inicio=(pagina-1)*por_pagina
    dash['transactions']=todas_txs[inicio:inicio+por_pagina]
    dash['total_transacoes_full']=len(todas_txs)  # total real para exibição
    dash['pagina']=pagina
...
def conciliacao():
    redir=require_login()
    if redir: return redir
    ctx=get_ctx(); dash=get_dash(session['user'])
    ...
    if dash['total_despesas']>dash['total_receitas']: recomendacoes.append('📉 Despesas superam receitas.')
    if conc['com_par']>0:          recomendacoes.append(f'🔗 {conc["com_par"]} transações com par identificado.')
    recomendacoes.append(f'✅ {conc["confirmados"]} de {conc["total"]} lançamentos confirmados.')
    return render_template('conciliacao.html', **ctx, active='conciliacao',
                           ...
                           percentual=conc['percentual'], com_par=conc['com_par'],
                           sem_par=conc['sem_par'], pares=conc['pares'],
...
def upload_file():
    ...
    if not file or file.filename=='':
        flash('Nenhum arquivo.','warning'); return redirect(url_for('dashboard'))
    if '.' in file.filename and file.filename.rsplit('.',1)[1].lower() in ALLOWED:
        ...
        fpath=os.path.join(UPLOAD_FOLDER,fname); file.save(fpath)
        users=load_users()
        if session['user'] not in users: users[session['user']]={'files':[]}
        users[session['user']].setdefault('files',[]).append(
            {'name':file.filename,'path':fpath,
             'type':file.filename.rsplit('.',1)[1].lower(),'date':datetime.now().isoformat()})
        save_users(users); add_log('Upload',session['user'],file.filename)
        txs=process_file(fpath)
        if txs: flash(f'"{file.filename}" importado com {len(txs)} transações!','success')
        else:   flash(f'"{file.filename}" salvo, mas nenhuma transação encontrada.','warning')
    else:
...
def configuracoes_usuario():
    ...
    if request.method=='POST':
        ...
        if acao=='alterar_email':
            ...
            if not novo_email:                                         error='Informe o novo e-mail.'
            elif novo_email in users and novo_email!=session['user']:  error='E-mail já em uso.'
            else:
                users[novo_email]=users.pop(session['user']); save_users(users)
                session['user']=novo_email; success='E-mail alterado!'
        elif acao=='alterar_senha':
            sa=request.form.get('senha_atual','')
            ns=request.form.get('nova_senha','')
            cs=request.form.get('confirmar_senha','')
...
def download_excel():
    redir=require_login()
    if redir: return redir
    return send_file(build_excel_conciliacao(get_dash(session['user'])),
                     mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True, download_name=f'FiscalPro_{datetime.now().strftime("%Y%m%d")}.xlsx')
...
def download_csv():
    ...
    return send_file(io.BytesIO(out.getvalue().encode('utf-8')), mimetype='text/csv',
                     as_attachment=True, download_name=f'FiscalPro_{datetime.now().strftime("%Y%m%d")}.csv')
...
def download_conciliacao():
    ...
    recs=['✅ Conciliação 100% concluída — todos os lançamentos confirmados.',
          '📊 Relatório gerado pelo FiscalPro — Sistema de Gestão Fiscal Inteligente']
    if dash['saldo']<0:
        recs.insert(0,'⚠️ Saldo negativo. Reveja suas despesas imediatamente.')
    out=build_excel_conciliacao(dash,recomendacoes=recs); out.seek(0)
    return send_file(out, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True, download_name=f'Conciliacao_{datetime.now().strftime("%Y%m%d")}.xlsx')
