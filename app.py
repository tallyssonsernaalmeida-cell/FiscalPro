...
def get_dash(email):
    ...
    return {
        'transacoes': txs, 'transactions': txs,
        'total_receitas': round(rec, 2), 'total_creditos': round(rec, 2),
        'total_despesas': round(desp, 2), 'total_debitos': round(desp, 2),
        'saldo': saldo,
...
def build_excel_conciliacao(data, recomendacoes=None):
    ...
    if cats_d:
        ...
        bar1.series[0].graphicalProperties.solidFill=R; ws.add_chart(bar1,f'G{cat_start-2}')
    r=cat_end+3
    ...
    if cats_r:
        ...
        pie.set_categories(Reference(ws,min_col=1,min_row=rec_start,max_row=rec_end)); ws.add_chart(pie,f'G{rec_end+2}')
    r=rec_end+20
        ...
        for id_, d in enumerate(desp_list):
            ...
            if abs(r['valor'] - abs(d['valor'])) < 0.01 and r['data'] == d['data'] and r['valor'] > 0:
                ...
                if comuns:
                    ...
                    break
    com_par = len(rec_pareados) + len(desp_pareados); sem_par = total - com_par
    ...
    if request.method == 'POST':
        ...
        if not all([nome, email, pwd]): error = 'Preencha todos os campos.'
            ...
            if email in users: error = 'E-mail já cadastrado.'
                ...
                save_users(users); add_log('Novo cadastro', email, f'IP: {request.remote_addr}')
...
def admin_dashboard():
    ...
    return render_template('admin_dashboard.html', **ctx, active='admin', usuarios=lista, total_usuarios=len(users), total_ativos=sum(1 for d in users.values() if d.get('ativo', True)), support_messages=load_support(), logs=logs_unicos[:30], site_url=request.host_url.rstrip('/'))
...
def dashboard():
    ...
    dash['pagina'] = pagina; dash['total_paginas'] = total_paginas
...
def conciliacao():
    ...
    return render_template('conciliacao.html', **ctx, active='conciliacao', creditos=rec, debitos=desp, total_creditos=sum(t['valor'] for t in rec), total_debitos=sum(abs(t['valor']) for t in desp), conciliados=conc['confirmados'], total_itens=conc['total'], percentual=conc['percentual'], recomendacoes=recomendacoes)
...
def upload_file():
    ...
    if '.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in ALLOWED:
        ...
        users[session['user']].setdefault('files', []).append({'name': file.filename, 'path': fpath, 'type': file.filename.rsplit('.', 1)[1].lower(), 'date': datetime.now().isoformat()})
        save_users(users); add_log('Upload', session['user'], file.filename)
        txs = process_file(fpath)
        if txs: flash(f'Arquivo "{file.filename}" importado com {len(txs)} transações!', 'success')
        else: flash(f'Arquivo "{file.filename}" salvo, mas nenhuma transação encontrada.', 'warning')
    else: flash('Formato não permitido.', 'danger')
    return redirect(url_for('dashboard'))
...
def remover_arquivo():
    redir = require_login()
    if redir: return redir
    filename = request.form.get('filename', ''); users = load_users(); nova = []
...
def configuracoes_usuario():
    ...
    if request.method == 'POST':
        ...
        if acao == 'alterar_email':
            ...
            elif novo_email in users and novo_email != session['user']: error = 'Este e-mail já está em uso.'
            else:
                users[novo_email] = users.pop(session['user']); save_users(users)
                session['user'] = novo_email; success = 'E-mail alterado com sucesso!'
        elif acao == 'alterar_senha':
            sa = request.form.get('senha_atual', ''); ns = request.form.get('nova_senha', ''); cs = request.form.get('confirmar_senha', '')
            if not check_password_hash(users[session['user']]['password'], sa): error = 'Senha atual incorreta.'
            elif ns != cs: error = 'Senhas não conferem.'
